...
export class Jogo {
    ...
    configurarNivel() {
        // Raquete
        const raqueteLarguraBase = 100; // Largura base da raquete em pixels
        // Largura responsiva, mas não maior que 25% da largura do canvas e não menor que 60px
        ...
        this.raquete = new Raquete(
            ...
            raqueteLargura,
            raqueteAltura,
            this.canvas // Passa a referência do canvas para a raquete
        );

        // Bola
        const raioBolaBase = 8; // Raio base da bola
        // Raio responsivo, com mínimo de 6px e máximo de 12px, ajustado pela largura do canvas
        const raioBola = Math.max(6, Math.min(raioBolaBase, this.canvas.width / 90, 12));
        const velocidadeBolaBase = 3; // Velocidade base
        // Ajusta velocidade à escala do canvas (considerando 900px como largura original para cálculo)
        // e aumenta com o nível
        const velocidadeBola = (velocidadeBolaBase + (this.nivel -1) * 0.5) * (this.canvas.width / 900);


        this.bolas = [new Bola(
            this.canvas.width / 2,
            this.canvas.height - 60 - raioBola, // Posição inicial da bola
            raioBola,
            velocidadeBola,
            this.canvas // Passa a referência do canvas para a bola
        )];
        
        this.tijolos = [];
        this.powerUps = [];

        // Reseta pontuação apenas se estiver no início do jogo (não ao reiniciar nível pausado ou avançar)
    ...
    criarTijolos() {
        this.tijolos = [];
        const linhasPorNivel = [3, 4, 5]; // Número de linhas de tijolos por nível
        const colunas = 10;

        // Calcula dimensões e posições baseadas no tamanho do canvas (originalmente 900x500)
        // Isso ajuda a manter a proporção dos tijolos em diferentes tamanhos de tela
        ...
        const coresNivel = [ // Paletas de cores por nível
            ['#FF4136', '#FF851B', '#FFDC00'], // Nível 1: Vermelho, Laranja, Amarelo
            ['#2ECC40', '#0074D9', '#B10DC9', '#FF851B'], // Nível 2: Verde, Azul, Roxo, Laranja
            ['#F012BE', '#3D9970', '#777777', '#0074D9', '#FFDC00']  // Nível 3: Magenta, Verde-Oliva, Cinza, Azul, Amarelo
        ];
    ...
    getTouchPos(canvasEl, evt) {
        ...
        // Prioriza touches (ativo), depois changedTouches (fim do toque)
        ...
        if (!touch) return null; // Se não houver informação de toque

        // Converte coordenadas da tela para coordenadas do canvas
        // Importante quando o canvas tem CSS para redimensionar e não corresponde ao width/height
    ...
    iniciarEventos() {
        ...
        // Touch events
        // Usar { passive: false } para permitir preventDefault() e evitar scroll/zoom indesejado
    ...
    tratarToque(e, tipoEvento) {
        ...
        if (!touchPos) return; // Sai se não conseguiu obter a posição do toque

        if (this.estado === 'jogando') {
            if (tipoEvento === 'start' || tipoEvento === 'move') {
                // Move a raquete para a posição X do toque, centralizando-a
        ...
        } else if (tipoEvento === 'end') { // Ações de menu e telas finais geralmente no 'touchend'
            if (this.estado === 'inicio' || this.estado === 'pausado') {
                ...
                const fontSizeOpcao = this.canvas.height / 20; // Tamanho da fonte para as opções
                let yPrimeiraOpcao = this.canvas.height / 2 - fontSizeTitulo * 0.2; // Posição Y da primeira opção
                // Se for menu de pausa, desce um pouco mais as opções por causa do título "PAUSADO"
                if (this.estado === 'pausado') yPrimeiraOpcao = this.canvas.height * 0.5;


                const alturaLinhaOpcao = fontSizeOpcao * 1.8; // Espaçamento entre as opções

                opcoes.forEach((opcao, i) => {
                    // Calcula a área de toque para cada opção
                    const textoMetrica = this.ctx.measureText(opcao); // Precisa do ctx para medir
                    // Área de toque mais generosa para facilitar em mobile
                ...
                // Qualquer toque nessas telas retorna ao menu inicial
                this.estado = 'inicio';
                this.nivel = 1; // Reseta o nível
                this.configurarNivel(); // Reconfigura para o nível 1
            }
        }
    }
    ...
    tratarKeyDown(e) {
        const key = e.code; // Usar e.code para layout independente
        const keyChar = e.key.toLowerCase(); // Para 'p' ou 'escape'
        ...
        if (this.estado === 'inicio') {
            ...
            if (key === 'ArrowLeft' || key === 'KeyA') this.raquete.mover('esquerda');
                ...
                this.estado = 'pausado';
                this.menuPausaSelecionado = 0; // Reseta seleção do menu de pausa
            }
            ...
            if (key === 'ArrowUp' || key === 'KeyW') {
                ...
                this.estado = 'jogando'; // Retorna ao jogo
            }
        } else if (this.estado === 'gameOver' || this.estado === 'vitoria' || this.estado === 'comoJogar') {
            // Qualquer tecla pressionada (exceto modificadores) retorna ao menu inicial
    ...
    executarAcaoMenuInicial() {
        ...
        if (opcao === 'Iniciar Jogo') {
            this.estado = 'jogando';
            this.nivel = 1; // Começa do nível 1
            this.configurarNivel(); // Configura o jogo para o nível 1
    ...
    executarAcaoMenuPausa() {
        ...
        if (opcao === 'Continuar') {
            ...
            this.estado = 'jogando';
            const pontuacaoAtual = this.pontuacao; // Salva a pontuação
            // Perde uma vida ao reiniciar, mas não menos que 1
            const vidasAtuais = this.vidas > 1 ? this.vidas -1 : 1; 
            this.configurarNivel(); // Reconfigura o nível atual
            this.pontuacao = pontuacaoAtual; // Restaura a pontuação
            this.vidas = vidasAtuais; // Define as vidas após penalidade
        } else if (opcao === 'Sair para Menu') {
            this.estado = 'inicio';
            this.nivel = 1; // Reseta para o nível 1
            this.configurarNivel(); // Reconfigura tudo
        }
    }
    ...
    atualizarLogica() {
        ...
        this.bolas.forEach(bola => {
            if (bola.ativa) {
                ...
                if (bola.verificarColisaoTijolos(this.tijolos, this)) {
                    // Chance de dropar um PowerUp ao destruir um tijolo
                    if (Math.random() < 0.2) { // 20% de chance de PowerUp
                        // Encontra o tijolo que acabou de ser destruído para usar sua posição
                        // Esta lógica pode ser aprimorada para pegar o tijolo exato da colisão
                        ...
                        if (tijoloDestruido) {
                            this.powerUps.push(new PowerUp(
                                ...
                                this.canvas // Passa a referência do canvas
                            ));
                            // Marca o tijolo para não dropar power-up de novo se houver outra verificação antes do filter
                            tijoloDestruido.cor = 'transparent_placeholder'; 
                        } else if (this.tijolos.length > 0 && this.tijolos.some(t => t.ativo)) {
                            // Fallback: se não encontrar o tijolo destruído (improvável com a flag),
                            // tenta pegar um tijolo ativo aleatório (menos ideal, mas garante o drop)
        ...
        // Remove tijolos inativos (aqueles que foram atingidos)
        this.tijolos = this.tijolos.filter(tijolo => tijolo.ativo);

        // Atualiza e verifica colisões dos PowerUps
        this.powerUps.forEach((powerUp, index) => {
            powerUp.atualizar();
            if (powerUp.posy > this.canvas.height) { // PowerUp saiu da tela
                this.powerUps.splice(index, 1);
            } else if (powerUp.verificarColisaoRaquete(this.raquete)) {
                powerUp.aplicarEfeito(this);
                this.powerUps.splice(index, 1); // Remove PowerUp após coleta
            }
        });

        // Verifica se ainda há bolas ativas
        ...
        if (bolasAtivas.length === 0) {
            ...
            if (this.vidas <= 0) {
                ...
                // Reposiciona a raquete e cria uma nova bola
                this.raquete.posx = this.canvas.width / 2 - this.raquete.largura / 2;
                const raioBola = parseFloat(this.bolas[0]?.raio || this.canvas.width / 90); // Usa raio da bola anterior ou default
                const velocidadeBola = parseFloat(this.bolas[0]?.velocidadeBase || (3 * (this.canvas.width / 900))); // Usa vel base anterior ou default
                this.bolas = [new Bola(
                    this.canvas.width / 2,
        ...
        this.bolas = bolasAtivas.length > 0 ? bolasAtivas : this.bolas; // Atualiza array de bolas


        // Verifica condição de vitória (nenhum tijolo restante)
        if (this.tijolos.length === 0 && this.estado === 'jogando') {
            this.nivel++;
            if (this.nivel > this.maxNiveis) {
                this.estado = 'vitoria';
            } else {
                // Salva pontuação e vidas antes de reconfigurar para o próximo nível
                const pontuacaoAtual = this.pontuacao;
                const vidasAtuais = this.vidas;
                this.configurarNivel(); // Configura para o próximo nível
                this.pontuacao = pontuacaoAtual; // Mantém a pontuação
                this.vidas = vidasAtuais; // Mantém as vidas
                // Adiciona um breve momento para o jogador se preparar
                this.estado = 'pausado'; // Ou um estado 'nivelIniciando'
                ...
                setTimeout(() => {
                    if(this.estado === 'pausado' || this.estado === 'jogando'){ // Só continua se não tiver ido para outro estado
                        this.estado = 'jogando';
                    }
    ...
    desenhar() {
        ...
        // Fonte base responsiva para menus e HUD
        ...
        if (this.estado === 'inicio') {
            ...
            this.tijolos.forEach(tijolo => tijolo.desenhar(this.ctx)); // Desenha apenas ativos por padrão na classe Tijolo
    ...
    desenharTelaComoJogar(fontSizeBase) {
        this.ctx.fillStyle = 'rgba(0, 0, 0, 0.85)'; // Fundo semi-transparente escuro
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
        
        const fontSizeTitulo = fontSizeBase * 1.8;
        const fontSizeTexto = fontSizeBase * 1.0;
        const lineHeight = fontSizeTexto * 1.6; // Espaçamento entre linhas
        let currentY = this.canvas.height * 0.25; // Posição Y inicial

        this.ctx.fillStyle = 'white';
        this.ctx.font = `bold ${fontSizeTitulo}px Arial`;
        this.ctx.textAlign = 'center';
        this.ctx.fillText("Como Jogar", this.canvas.width / 2, currentY);
        currentY += lineHeight * 2; // Espaço após o título
        ...
        currentY += lineHeight * 1.5; // Espaço extra
        
        this.ctx.font = `italic ${fontSizeTexto * 0.9}px Arial`;
        this.ctx.fillStyle = '#CCCCCC'; // Cor mais clara para os controles
        ...
        currentY += lineHeight * 2; // Espaço antes da instrução final

        this.ctx.fillStyle = '#FFFF88'; // Amarelo claro para destaque
    ...
    desenharHUD(fontSize) {
        ...
        this.ctx.fillText(`Pontuação: ${this.pontuacao}`, 10, fontSize * 1.1); // Posição Y ajustada pela fonte
    ...
    desenharMenu(opcoes, selecionado, titulo, instrucao, fontSizeBase) {
        ...
        // Tamanhos de fonte relativos à base
        ...
        opcoes.forEach((opcao, i) => {
            const yPos = this.canvas.height * 0.5 + i * (fontSizeOpcao * 1.8); // Espaçamento entre opções
            if (i === selecionado) {
                this.ctx.fillStyle = 'yellow'; // Destaque para opção selecionada
    ...
    desenharTelaFinal(mensagemPrincipal, mensagemSecundaria, instrucao, fontSizeBase) {
        ...
        // Mensagem Principal (Game Over / Vitória)
        this.ctx.fillStyle = 'white';
        ...
        this.ctx.fillStyle = '#FFFF99'; // Amarelo claro para a instrução
